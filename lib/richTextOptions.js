...
// Merge tags: nt_mergetag entries embedded inline by an editor, e.g.
// "a three hour flight from {city}". Note "inline entry" literally — typing
// braces as text renders braces, since they are only characters.
//
// article.articleContent, page.pageInformation and greetingBanner.message all
// restrict inline entries to this one type.
export const renderInlineMergeTag = (node) => {
  const target = node.data.target;

  if (target?.sys?.contentType?.sys?.id !== MERGE_TAG_CONTENT_TYPE) {
    return null;
  }
  ...
  return (
    <ProfileValue
      ...
      fallback={target.fields.nt_fallback}
    />
  );
};
...
export const richTextOptions = {
  ...
  renderNode: {
    ...
    [BLOCKS.UL_LIST]: (node, children) => {
      return <ul className="mb-4 list-disc list-inside space-y-1">{children}</ul>;
    },
    [BLOCKS.LIST_ITEM]: (node, children) => {
      return <li className="text-xl [&>p]:mb-0 [&>p]:inline">{children}</li>;
    },
    [BLOCKS.OL_LIST]: (node, children) => {
      return <ol className="mb-4 list-decimal list-inside space-y-1">{children}</ol>;
    },
    [BLOCKS.PARAGRAPH]: (node, children) => {
      return <p className="mb-4 text-xl">{children}</p>;
    },
    ...
    [INLINES.EMBEDDED_ENTRY]: renderInlineMergeTag,
...
// The banner is a single line inside its own strip, so paragraphs render as
// bare children rather than the spaced, text-xl <p> the body copy uses.
export const bannerRichTextOptions = {
  renderNode: {
